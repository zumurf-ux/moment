...
const menus = [["dashboard","운영 현황"],["editions","발행본 편집"],["candidates","이슈 후보"],["audio","음성 자산"],["sources","수집 소스"],["audit","감사 로그"],["settings","서비스 설정"]];
...
async function loadFirebaseData(){
  const {collection,getDocs,doc,getDoc}=state.api; const names=["editions","candidates","sources","audioAssets","auditLogs"];
...
function audio(){ const edition=state.data.editions.find(x=>x.id===state.selectedEdition)||state.data.editions[0]; return head("음성 자산","브라우저 TTS로 원고를 확인하고 Firebase Storage URL을 관리합니다.",`<button class="secondary" data-action="speak">미리듣기</button><button class="primary" data-action="save-audio">원고 저장</button>`)+`<section class="grid two-col"><article class="card"><h2>음성 원고</h2><label>발행본<select id="audioEdition">${state.data.editions.map(x=>`<option value="${x.id}" ${x.id===edition?.id?'selected':''}>${esc(x.publishDate)} · v${x.version}</option>`).join("")}</select></label><label>원고<textarea id="audioScript">${esc(edition?.audioScript||"")}</textarea></label><label>Storage 다운로드 URL<input id="audioUrl" value="${esc(edition?.audioUrl||"")}" placeholder="https://firebasestorage.googleapis.com/..."></label></article><article class="card"><h2>생성된 음성</h2>${state.data.audioAssets.map(x=>`<div class="status-row"><span>${esc(x.provider)} · ${esc(x.voice)}</span>${badge(x.status)}</div>`).join("")||'<p class="empty">음성 자산 없음</p>'}</article></section>`; }
...
function render(){ renderNav(); const views={dashboard,editions,candidates,audio,sources,audit:auditView,settings}; $("#content").innerHTML=views[state.view](); bindContent(); $("#content").focus(); }
...
async function handleAction(action){
  try{
    ...
    if(action==="speak"){ speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance($("#audioScript").value);u.lang="ko-KR";speechSynthesis.speak(u);toast("음성 미리듣기를 시작했습니다.");return; }
    if(action==="save-audio"){ const id=$("#audioEdition").value; await saveDoc("editions",id,{audioScript:$("#audioScript").value,audioUrl:$("#audioUrl").value}); const assetId=`audio-${id}`;await saveDoc("audioAssets",assetId,{editionId:id,provider:"Firebase Storage",voice:"ko-KR",version:1,status:$("#audioUrl").value?"READY":"SCRIPT_READY"});toast("음성 정보를 저장했습니다."); }
