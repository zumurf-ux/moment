...
import { demoData } from "./demo-data.js?v=6";
...
const sourceTypeLabels = { OFFICIAL:"공식", NEWS:"뉴스", TREND:"트렌드" };
const methodLabels = { API:"자동 연동", RSS:"새 소식 모음", MANUAL:"수동" };
const gradeLabels = { A:"가", B:"나", C:"다" };
const actionLabels = { "system.seeded":"초기 데이터 생성", "config.updated":"서비스 설정 변경", "document.updated":"문서 변경", "account.credentials_updated":"운영자 계정 변경" };
...
function dashboard(){ const d=state.data; return head("운영 현황","파이어베이스 실시간 운영 데이터",`<button class="primary" data-action="pipeline">수집·분석 갱신</button>`)+`<section class="grid metrics"><div class="metric"><span>공개 발행본</span><strong>${d.editions.filter(x=>x.status==="PUBLISHED").length}</strong></div><div class="metric"><span>검수 대기</span><strong>${d.editions.filter(x=>x.status==="IN_REVIEW").length}</strong></div><div class="metric"><span>후보 묶음</span><strong>${d.candidates.length}</strong></div><div class="metric"><span>활성 소스</span><strong>${d.sources.filter(x=>x.active).length}</strong></div></section><section class="grid two-col"><article class="card"><h2>오늘 운영 상태</h2><div class="status-list"><div class="status-row"><span>최신 공개일</span><strong>${esc(d.editions.find(x=>x.status==="PUBLISHED")?.publishDate||"-")}</strong></div><div class="status-row"><span>수집 오류</span><strong>${d.sources.filter(x=>x.lastStatus==="ERROR").length}건</strong></div><div class="status-row"><span>자료 저장소</span><strong>${configured?"파이어베이스":"데모"}</strong></div></div></article><article class="card"><h2>최근 활동</h2>${d.auditLogs.slice(0,6).map(x=>`<div class="status-row"><span>${esc(actionLabel(x.action))}</span><small>${formatDate(x.createdAt)}</small></div>`).join("")||'<p class="empty">기록 없음</p>'}</article></section>`; }
function editions(){ const current=state.data.editions.find(x=>x.id===state.selectedEdition)||state.data.editions[0]; if(!current)return head("발행본 편집","후보를 선택해 첫 발행본을 만드세요.")+`<div class="empty">발행본이 없습니다.</div>`; const checks=["수치·고유명사 확인","공식 또는 원문 출처","발표일·시행일 확인","사실·전망 분리","정파 평가 없음","상반된 주장 동등 표기","주어·대상 명시","수치 단위 명시","인용 주체 명시","출처 연결 확인","휴대전화 미리보기"]; return head("발행본 편집","확인된 큰 이슈를 정파 평가 없이 사실 문장으로만 발행합니다.",`<button class="secondary" data-action="save-edition">저장</button><button class="primary" data-action="approve">승인</button><button class="danger" data-action="withdraw">철회</button>`)+`<section class="split"><div class="list">${state.data.editions.map(x=>`<button class="list-button ${x.id===current.id?'active':''}" data-edition="${x.id}"><strong>${esc(x.publishDate)} · ${esc(typeLabel(x.type))}</strong><span>${x.version}판 · ${esc(statusLabel(x.status))}</span></button>`).join("")}</div><div class="grid"><article class="card"><div class="form-grid"><label>제목<input id="editionHeadline" value="${esc(current.headline)}"></label><label>발행일<input id="editionDate" type="date" value="${esc(current.publishDate)}"></label><label>상태<select id="editionStatus">${["DRAFT","IN_REVIEW","APPROVED","SCHEDULED","PUBLISHED","WITHDRAWN"].map(x=>`<option value="${x}" ${x===current.status?'selected':''}>${statusLabel(x)}</option>`).join("")}</select></label><label>판 번호<input value="${current.version}" disabled></label></div></article><article class="card"><h2>핵심 팩트 ${current.items?.length||0}개</h2>${(current.items||[]).map((x,i)=>`<div class="candidate"><span>${String(i+1).padStart(2,"0")}</span><div><h3>${esc(x.title)}</h3><p>${esc(x.summary)}</p>${x.sourceName?`<p class="fact-source">확인 · ${esc(x.factDate||"발표일 미기재")} · <a href="${esc(x.sourceUrl||"#")}" target="_blank" rel="noreferrer">${esc(x.sourceName)}</a></p>`:""}</div><span class="badge">${esc(x.category)}</span></div>`).join("")}</article><article class="card"><h2>사실·정치 중립 검수 11개</h2><div class="checklist">${checks.map((x,i)=>`<label class="check"><input type="checkbox" class="review-check" ${current.reviewChecklist?.[i]?'checked':''}>${x}</label>`).join("")}</div></article></div></section>`; }
function candidates(){ return head("이슈 후보","활성 소스에서 생성된 편집 후보입니다.",`<button class="secondary" data-action="pipeline">후보 갱신</button><button class="primary" data-action="create-draft">선택 후보로 초안 생성</button>`)+`<article class="card">${state.data.candidates.map(x=>`<div class="candidate"><input type="checkbox" data-candidate="${x.id}" ${state.selectedCandidates.has(x.id)?'checked':''}><div><h3>${esc(x.title)}</h3><p>${esc(x.summary)} · 출처 ${(x.sourceIds||[]).length}개</p></div><span class="badge">신뢰 ${esc(gradeLabels[x.trustGrade]||x.trustGrade||"나")}등급 · ${x.score||0}점</span></div>`).join("")||'<p class="empty">후보가 없습니다.</p>'}</article>`; }
function sources(){ return head("수집 소스","공식·뉴스·트렌드 소스를 관리합니다.")+`<article class="card"><form id="sourceForm" class="source-form"><input id="sourceName" placeholder="소스 이름" required><select id="sourceType"><option value="OFFICIAL">공식</option><option value="NEWS">뉴스</option><option value="TREND">트렌드</option></select><select id="sourceMethod"><option value="API">자동 연동</option><option value="RSS">새 소식 모음</option><option value="MANUAL">수동</option></select><button class="primary">추가</button></form><div class="table-wrap"><table><thead><tr><th>이름</th><th>유형</th><th>신뢰</th><th>수집</th><th>상태</th><th>활성</th></tr></thead><tbody>${state.data.sources.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(sourceTypeLabels[x.sourceType]||x.sourceType)}</td><td>${esc(gradeLabels[x.trustLevel]||x.trustLevel)}등급</td><td>${esc(methodLabels[x.collectionMethod]||x.collectionMethod)}</td><td>${badge(x.lastStatus||"READY")}</td><td><label class="toggle"><input type="checkbox" data-source="${x.id}" ${x.active?'checked':''}>사용</label></td></tr>`).join("")}</tbody></table></div></article>`; }
...
async function handleAction(action){
  try{
    ...
    if(action==="create-draft"){ const items=state.data.candidates.filter(x=>state.selectedCandidates.has(x.id)).map((x,i)=>({order:i+1,category:x.category,title:x.title,summary:x.summary,sourceName:x.sourceName||"",sourceUrl:x.sourceUrl||"",factDate:x.factDate||""})); if(!items.length)throw new Error("후보를 하나 이상 선택하세요."); const id=`daily-${new Date().toISOString().slice(0,10)}-${crypto.randomUUID().slice(0,5)}`; await saveDoc("editions",id,{type:"daily",publishDate:new Date().toISOString().slice(0,10),version:1,status:"DRAFT",headline:"오늘 반드시 알아야 할 핵심 팩트",items,sourceCount:items.length*2}); state.selectedEdition=id; state.view="editions"; toast("초안을 만들었습니다."); }
