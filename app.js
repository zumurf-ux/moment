...
import { demoData } from "./demo-data.js?v=10";
...
function expectedSourceDate(){ return previousDate(activePublishDate()); }
...
function candidates(){ const ranked=rankedCandidates(); return head("이슈 후보",`${expectedSourceDate()} 00:00~23:59에 발행되고 사실 검증을 통과한 후보만 표시합니다.`,`<button class="secondary" data-action="sync-previous-day">전날판 데이터 반영</button><button class="secondary" data-action="pipeline">영향도 재분석</button><button class="primary" data-action="create-draft">상위 후보로 초안 생성</button>`)+`<div class="policy-note"><strong>일간판 수집 기준 · ${esc(expectedSourceDate())}</strong><p>전날 발행 확인 필수 · 최신성 25% · 국민 영향도 30% · 안전성 25% · 출처 검증도 20%. 다른 날짜 자료와 미검증 후보는 자동 제외됩니다.</p></div><article class="card">${ranked.map((x,index)=>`<div class="candidate"><input type="checkbox" data-candidate="${x.id}" ${state.selectedCandidates.has(x.id)?'checked':''}><div><h3>${index+1}순위 · ${esc(x.title)}</h3><p>${esc(x.summary)} · 발행일 ${esc(x.sourceDate)} · 출처 ${(x.sourceIds||[]).length}개</p><p class="fact-source">선정 근거 · ${esc(x.analysisReason)}</p></div><span class="badge">선정 ${x.analysisScore}점 · 신뢰 ${esc(gradeLabels[x.trustGrade]||x.trustGrade||"나")}등급</span></div>`).join("")||'<p class="empty">전날 발행·검증 조건을 충족한 후보가 없습니다.</p>'}</article>`; }
...
async function handleAction(action){
  try{
    ...
    if(action==="create-draft"){ const ranked=rankedCandidates(); const pool=state.selectedCandidates.size?ranked.filter(x=>state.selectedCandidates.has(x.id)):ranked.slice(0,6); const items=pool.map((x,i)=>({order:i+1,category:x.category,title:x.title,summary:x.summary,sourceName:x.sourceName||"",sourceUrl:x.sourceUrl||"",factDate:x.factDate||"",sourceDate:x.sourceDate||"",isHot:i===0,selectionScore:x.analysisScore,selectionReason:x.analysisReason})); if(!items.length)throw new Error(`${expectedSourceDate()}에 발행되고 검증된 후보가 없습니다.`); const publishDate=localIsoDate(); const sourceDate=previousDate(publishDate); const id=`daily-${publishDate}-${crypto.randomUUID().slice(0,5)}`; await saveDoc("editions",id,{type:"daily",publishDate,sourceDate,sourceWindowStart:`${sourceDate}T00:00:00+09:00`,sourceWindowEnd:`${sourceDate}T23:59:59+09:00`,version:1,status:"DRAFT",headline:`${Number(sourceDate.slice(5,7))}월 ${Number(sourceDate.slice(8,10))}일 가장 영향도가 큰 6가지 사실`,items,sourceCount:new Set(pool.flatMap(x=>x.sourceIds||[])).size,selectionModel:"일간 영향도 분석 v2",selectionFactors:"전날 발행 확인·최신성 25%·영향도 30%·안전성 25%·검증도 20%"}); state.selectedEdition=id; state.view="editions"; toast(`${sourceDate} 이슈만으로 초안을 만들었습니다.`); }
