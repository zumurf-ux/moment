...
import { demoData } from "./demo-data.js?v=9";
...
function rankedCandidates(){ return state.data.candidates.map(analyzeCandidate).sort((a,b)=>b.analysisScore-a.analysisScore||String(a.title).localeCompare(String(b.title),"ko")); }
...
async function handleAction(action){
  try{
    ...
    if(action==="create-draft"){ const ranked=rankedCandidates(); const pool=state.selectedCandidates.size?ranked.filter(x=>state.selectedCandidates.has(x.id)):ranked.slice(0,6); const items=pool.map((x,i)=>({order:i+1,category:x.category,title:x.title,summary:x.summary,sourceName:x.sourceName||"",sourceUrl:x.sourceUrl||"",factDate:x.factDate||"",isHot:i===0,selectionScore:x.analysisScore,selectionReason:x.analysisReason})); if(!items.length)throw new Error("분석할 후보가 없습니다."); const id=`daily-${new Date().toISOString().slice(0,10)}-${crypto.randomUUID().slice(0,5)}`; await saveDoc("editions",id,{type:"daily",publishDate:new Date().toISOString().slice(0,10),version:1,status:"DRAFT",headline:"오늘 영향도가 가장 큰 6가지 사실",items,sourceCount:new Set(pool.flatMap(x=>x.sourceIds||[])).size,selectionModel:"영향도 분석 v1",selectionFactors:"최신성 25%·영향도 30%·안전성 25%·검증도 20%"}); state.selectedEdition=id; state.view="editions"; toast("분석 순위대로 초안을 만들었습니다."); }
    ...
    if(action==="approve"){ const checks=[...document.querySelectorAll(".review-check")]; if(checks.some(x=>!x.checked))throw new Error("12개 검수 항목을 모두 확인하세요."); await saveDoc("editions",edition.id,{status:"APPROVED",reviewedAt:now(),reviewChecklist:checks.map(()=>true)}); toast("승인했습니다."); }
