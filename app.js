...
const sourceTypeLabels = { OFFICIAL:"공식 기관" };
...
function sources(){ return head("공식 자료 소스","국가기관·공공기관이 직접 제공하는 자료만 관리합니다.")+`<div class="policy-note"><strong>공공정보 전용 수집 원칙</strong><p>민간 언론사·뉴스 집계 서비스·민간 포털·제3자 중계 피드는 사용하지 않습니다. 공공누리 제0·1유형 또는 국가기관이 직접 공개한 자료만 허용합니다.</p></div><article class="card"><form id="sourceForm" class="source-form"><input id="sourceName" placeholder="공식 기관명" required><input id="sourceType" type="hidden" value="OFFICIAL"><select id="sourceMethod"><option value="RSS">공식 RSS</option><option value="API">공식 API</option><option value="MANUAL">공식 자료 수동 확인</option></select><button class="primary">추가</button></form><div class="table-wrap"><table><thead><tr><th>기관명</th><th>유형</th><th>신뢰</th><th>수집</th><th>상태</th><th>활성</th></tr></thead><tbody>${state.data.sources.filter(x=>x.sourceType==="OFFICIAL").map(x=>`<tr><td>${esc(x.name)}</td><td>공식 기관</td><td>${esc(gradeLabels[x.trustLevel]||x.trustLevel)}등급</td><td>${esc(methodLabels[x.collectionMethod]||x.collectionMethod)}</td><td>${badge(x.lastStatus||"READY")}</td><td><label class="toggle"><input type="checkbox" data-source="${x.id}" ${x.active?'checked':''}>사용</label></td></tr>`).join("")}</tbody></table></div></article>`; }
